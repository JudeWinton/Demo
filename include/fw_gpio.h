...
// 定义了一个名为GPIO_Mode_t的枚举类型，用于表示GPIO（通用输入/输出）的工作模式。
typedef enum
{
    GPIO_Mode_InOut_QBD  = 0x00, /* quasi-bidirectional（准双向模式）（兼容传统的8051系列MCU） */
    GPIO_Mode_Output_PP  = 0x01, /* push-pull output（推挽输出模式） */
    GPIO_Mode_Input_HIP  = 0x02, /* high-impedance input（高阻抗输入模式） */
    GPIO_Mode_InOut_OD   = 0x03, /* open drain output（开漏输出模式） */
} GPIO_Mode_t;
...
// 定义一个名为 GPIO_SchmittTrigger_t 的枚举类型，用于表示 GPIO 的施密特触发器（Schmitt Trigger）状态。
typedef enum
{
    GPIO_SchmittTrigger_ON  = 0x00,  // 施密特触发器开启
    GPIO_SchmittTrigger_OFF = 0x01,  // 施密特触发器关闭
} GPIO_SchmittTrigger_t;

// 定义一个名为 GPIO_SwitchSpeed_t 的枚举类型，用于表示 GPIO 的开关速度。
typedef enum
{
    GPIO_SwitchSpeed_High   = 0x00,  // 高速开关
    GPIO_SwitchSpeed_Low    = 0x01,  // 低速开关
} GPIO_SwitchSpeed_t;

// 定义一个名为 GPIO_DriveCapability_t 的枚举类型，用于表示 GPIO 的驱动能力。
typedef enum
{
    GPIO_DriveCapability_High      = 0x00,  // 高驱动能力
    GPIO_DriveCapability_Normal    = 0x01,  // 正常驱动能力
